...
export default function PracticalInterviewPage() {
  ...
  // 선택된 지원자의 면접 데이터 로딩
  const { questions, application, loading: dataLoading, error: dataError } = useInterviewData('practical', selectedCandidateId);
  ...
  useEffect(() => {
    const fetchCandidates = async () => {
      try {
        setListLoading(true);
        // 실무진 면접 대상자 조회
        const data = await getPracticalCandidates(); 
        setCandidates(data);
      } catch (err) {
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      >
        <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-blue-50">
          <h2 className="text-lg font-bold text-gray-800">실무진 면접 대기</h2>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        {selectedCandidateId ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-gray-50 text-gray-500">
            ...
            <p className="mt-2 text-sm">실무진 면접 대상자만 표시됩니다.</p>
          </div>
        )}
      </div>
    </div>
