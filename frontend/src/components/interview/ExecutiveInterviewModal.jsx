...
const ExecutiveInterviewModal = ({ isOpen, onClose, jobPostId, jobPost }) => {
  ...
  // 면접 상태별 라벨 반환 함수
  const getInterviewStatusLabel = (secondStatus, finalStatus) => {
    if (!secondStatus) {
      return {
        label: '미진행',
    ...
    let finalLabel = statusLabels[secondStatus]?.label || '알 수 없음';
    let finalColor = statusLabels[secondStatus]?.color || 'text-gray-500 bg-gray-100';
    
    if (secondStatus === 'PENDING') {
      finalLabel = '미진행';
      finalColor = 'text-gray-500 bg-gray-100';
    } else if (secondStatus === 'FAILED') {
      finalLabel = '불합격';
      finalColor = 'text-red-600 bg-red-100';
    } else if (secondStatus === 'PASSED') {
      finalLabel = '합격';
  ...
  const filteredApplicants = applicants.filter(applicant => {
    ...
    const matchesStatus = filterStatus === 'all' || 
                         (filterStatus === 'pending' && (!applicant.executive_interview_status || applicant.executive_interview_status === 'PENDING')) ||
                         (filterStatus === 'in_progress' && applicant.executive_interview_status?.includes('IN_PROGRESS')) ||
                         (filterStatus === 'completed' && applicant.executive_interview_status?.includes('COMPLETED')) ||
                         (filterStatus === 'passed' && applicant.executive_interview_status?.includes('PASSED')) ||
                         (filterStatus === 'failed' && applicant.executive_interview_status?.includes('FAILED'));
  ...
  const pendingApplicants = applicants.filter(a => !a.executive_interview_status || a.executive_interview_status === 'PENDING').length;
  const inProgressApplicants = applicants.filter(a => a.executive_interview_status?.includes('IN_PROGRESS')).length;
  const completedApplicants = applicants.filter(a => a.executive_interview_status?.includes('COMPLETED')).length;
  const passedApplicants = applicants.filter(a => a.executive_interview_status?.includes('PASSED')).length;
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredApplicants.map((applicant, index) => {
                const statusInfo = getInterviewStatusLabel(applicant.executive_interview_status, applicant.final_status);
