...
const PracticalInterviewModal = ({ isOpen, onClose, jobPostId, jobPost }) => {
  ...
  const filteredApplicants = applicants.filter(applicant => {
    ...
    const matchesStatus = filterStatus === 'all' || 
                         (filterStatus === 'pending' && (!applicant.practical_interview_status || applicant.practical_interview_status === 'PENDING')) ||
                         (filterStatus === 'in_progress' && applicant.practical_interview_status?.includes('IN_PROGRESS')) ||
                         (filterStatus === 'completed' && applicant.practical_interview_status?.includes('COMPLETED')) ||
                         (filterStatus === 'passed' && applicant.practical_interview_status?.includes('PASSED')) ||
                         (filterStatus === 'failed' && applicant.practical_interview_status?.includes('FAILED'));
  ...
  const pendingApplicants = applicants.filter(a => !a.practical_interview_status || a.practical_interview_status === 'PENDING').length;
  const inProgressApplicants = applicants.filter(a => a.practical_interview_status?.includes('IN_PROGRESS')).length;
  const completedApplicants = applicants.filter(a => a.practical_interview_status?.includes('COMPLETED')).length;
  const passedApplicants = applicants.filter(a => a.practical_interview_status?.includes('PASSED')).length;
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredApplicants.map((applicant, index) => {
                const statusInfo = getInterviewStatusLabel(applicant.practical_interview_status, applicant.executive_interview_status);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      {applicant.practical_interview_score && (
                        <div className="flex justify-between">
                          <span className="text-gray-500 dark:text-gray-400">평가점수:</span>
                          <span className="text-blue-600 dark:text-blue-400 font-semibold">
                            {applicant.practical_interview_score}점
                          </span>
                        </div>
                      )}
                    </div>
