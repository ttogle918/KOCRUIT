# 이력서 분석 시스템 - 핵심 백엔드 아키텍처 (통계시각화 제외)

## 🚪 백엔드 API 라우터 구조 (핵심 기능)
...
graph TB
    ...
    class AUTH,APPLICATIONS,RESUMES,HIGHLIGHT,AI_EVALUATE,INTERVIEW,REALTIME,COMPANIES,JOBS,SCHEDULES,NOTIFICATIONS router
...
graph TB
    ...
    class FASTAPI,API_ROUTER,AUTH,APPLICATIONS,HIGHLIGHT_API,AI_EVALUATE,INTERVIEW,REALTIME gateway
    ...
    class HIGHLIGHT_RESULT,ANALYSIS_RESULT,CACHE_RESULT output
```

## 🔄 핵심 데이터 플로우 시퀀스 다이어그램
...
이 다이어그램에서는 다음 통계시각화 관련 컴포넌트들을 제외했습니다:

- `statistics_analysis.py` API
- `applicantStats.js` 유틸리티
- 차트 컴포넌트들 (BarChart, PieChart, LineChart, ProvinceMapChart)
- `StatisticsAnalysis` 컴포넌트
- 통계 데이터 처리 로직

이를 통해 이력서 분석의 핵심 기능에만 집중한 아키텍처를 제공합니다. 