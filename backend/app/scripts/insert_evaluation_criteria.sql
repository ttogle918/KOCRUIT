...
-- 배치 삽입을 위한 프로시저 (여러 지원자용)
DELIMITER //

DROP PROCEDURE IF EXISTS InsertEvaluationCriteriaForApplicants //

CREATE PROCEDURE InsertEvaluationCriteriaForApplicants(
    IN job_post_id_param INT
)
BEGIN
    ...
    DECLARE cur CURSOR FOR 
        SELECT a.id, a.resume_id 
        FROM application a 
        WHERE a.job_post_id = job_post_id_param 
    ...
    read_loop: LOOP
        FETCH cur INTO app_id, resume_id_val;
        IF done THEN
            LEAVE read_loop;
        END IF;
...
-- 프로시저 사용 예시:
-- CALL InsertEvaluationCriteriaForApplicants(17); -- job_post_id = 17인 공고의 모든 지원자에게 평가 기준 삽입 