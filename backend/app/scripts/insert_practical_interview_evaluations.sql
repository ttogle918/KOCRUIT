-- AI 면접 통과자들의 실무진 면접 평가 더미 데이터 생성
-- 실제 평가 결과 형태로 데이터 삽입
...
SELECT 
    ...
    CASE 
        WHEN (SELECT COUNT(*) FROM company_user) > 0 THEN (SELECT cu.id FROM company_user cu ORDER BY RAND() LIMIT 1)
        ELSE NULL
    END as evaluator_id,  -- 실제 존재하는 면접관 중 랜덤, 없으면 NULL
    FALSE as is_ai,
...
JOIN application a ON ie.interview_id = (
    SELECT sia.schedule_interview_id 
    FROM schedule_interview_applicant sia 
    WHERE sia.user_id = a.user_id
)
...
FROM interview_evaluation ie
WHERE ie.evaluation_type = 'PRACTICAL'
GROUP BY ie.evaluator_id
ORDER BY ie.evaluator_id; 