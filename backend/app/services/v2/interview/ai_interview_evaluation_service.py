...
def save_ai_interview_evaluation(db: Session, application_id: int, interview_id: int = None, job_post_id: int = None, analysis: dict = None, json_path: str = None):
    ...
    if application:
        ...
        update_stage_status(
            db, application.id, StageName.AI_INTERVIEW, status, score=total_score
        )
        
        # 다음 단계(실무 면접)로 전이 (합격 시)
        if passed:
            application.current_stage = StageName.PRACTICAL_INTERVIEW
            update_stage_status(db, application.id, StageName.PRACTICAL_INTERVIEW, StageStatus.PENDING)
