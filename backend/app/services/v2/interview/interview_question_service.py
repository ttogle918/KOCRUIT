...
class InterviewQuestionService:
    ...
    def generate_questions_for_scheduled_interviews(
        ...
        try:
            # 면접 일정이 확정된 지원자들 조회 (ApplicationStage 활용)
            # 조건: 서류 합격(PASSED) AND (면접 스케줄이 잡힌 상태)
            DocStage = aliased(ApplicationStage)
            InterviewStage = aliased(ApplicationStage)
            
            scheduled_applications = db.query(Application).join(
                DocStage, Application.id == DocStage.application_id
            ).join(
                InterviewStage, Application.id == InterviewStage.application_id
            ).filter(
                Application.job_post_id == job_post_id,
                DocStage.stage_name == StageName.DOCUMENT,
                DocStage.status == StageStatus.PASSED,
                InterviewStage.stage_name.in_([StageName.AI_INTERVIEW, StageName.PRACTICAL_INTERVIEW, StageName.EXECUTIVE_INTERVIEW]),
                InterviewStage.status.in_([StageStatus.PENDING, StageStatus.SCHEDULED])
            ).distinct().all()
